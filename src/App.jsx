...
function MainApp({ cfg, setCfg, me }) {
  ...
  const scoutSubmit = (reqId) => { upsertProgress(me.id, reqId, { status: "Αναμένει έγκριση", completed_date: today() }, "Υποβλήθηκε για έγκριση"); logAction("Υπέβαλε αίτημα έγκρισης"); };
...
/* ---------------- ΠΡΟΣΚΟΠΟΣ ---------------- */
function ScoutView({ cfg, user, requirements, getProgressFor, onStart, onSubmit }) {
            ...
            {Object.entries(byCategory).map(([cat, list]) => (
              <div key={cat} style={{ marginBottom: 16 }}>
                ...
                <div style={{ display: "flex", flexDirection: "column", gap: 8 }}>
                  {list.map((r) => {
                    const p = getProgressFor(user.id, r.id);
                      ...
                      <Card key={r.id} style={{ padding: "12px 14px" }}>
                        <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", gap: 10, flexWrap: "wrap" }}>
                          ...
                          <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
                            ...
                            {status === "Σε εξέλιξη" && <Button cfg={cfg} small onClick={() => onSubmit(r.id)}>Υποβολή</Button>}
...
/* ---------------- ΑΙΤΗΜΑΤΑ ---------------- */
function RequestsView({ cfg, users, requirements, progress, scopeScouts, onDecide, onManualAdd }) {
...
function AdminOverview({ cfg, users, requirements, progress }) {
  ...
  return (
    <div>
      <h2 style={{ fontSize: 18, marginBottom: 12 }}>Σύνοψη</h2>
...
function AdminCatalog({ cfg, requirements, onAdd, onDelete }) {
